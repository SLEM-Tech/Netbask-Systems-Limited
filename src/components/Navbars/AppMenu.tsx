 ...
 * AppMenu Component
 *
 * Provides a persistent bottom navigation bar for mobile users (hidden on sm screens and larger).
 * Includes links to Home, Shop/Orders, Cart (with badge), and Account/Login.
 */
const AppMenu = () => {
  ...
  // State management for cart drawer and user menu visibility
  ...
  // Toggle functions for the cart drawer
   ...
   * @param onClick - Optional override callback
   */
  ...
  // Fetch orders to determine if "Shop" should change to "Orders"
  const {
    data: ordersData,
  ...
  /**
   * Configuration for mobile navigation links.
   * Icons and labels change dynamically based on user state (logged in, has orders).
   */
  const mobileHeaderLinkUrl = [
    ...
    {
      id: "shop",
      // If user has orders, show "Orders" link; otherwise, show "Shop"
    ...
    {
      id: "account",
      // If logged in, show "Account" link; otherwise, show "Login"
  ...
  return (
    <>
      {/* 
				...
				Visible only on small screens (max-width: 639px)
			*/}
      <div className="sm:hidden fixed bottom-0 z-50 w-full">
        ...
        <div className="relative bg-white border-t border-gray-300/50 backdrop-blur-sm">
            ...
            {mobileHeaderLinkUrl.map((menuItem) => {
              ...
              return (
                <button
                  ...
                  {/* Icon Container with highlight for active state */}
                  <div
          ...
          {/* 
						User Dropdown Menu
						Displayed when the user menu is toggled (currently handled via navigation in this version)
					*/}
          {isUserMenuOpen && (
      ...
      {/* Cart Drawer triggered by the menu button */}
      <Drawer
        open={isCartOpen}
        onClose={onCloseCart}
        placement="right"
      ...
      {/* Background overlay for the user menu when open */}
      {isUserMenuOpen && (
        <div
